...
export default (C) => connect(state => state, mapDispatchToProps)(class AuthComponent extends Component {
  ...
  componentDidMount() {
    ...
    const fetchData = async () => {
      ...
      if (decoded) if (decoded.id) {
        this.props.userActions.setId(decoded.id);
        this.props.userActions.fetchUserData(decoded.id);
      }
    }
    ...
    client.on('sendData', () => client.emit('userData', {
      id,
      isOnline: true
    }));
